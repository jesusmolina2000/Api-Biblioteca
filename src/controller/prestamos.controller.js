	...
	try {
		conexion.query(`SELECT * FROM prestamos WHERE id = ${req.params.id}`, (err, rows, fields) => {
			if (!err) {
				...
				res.json({ status: 'No existe el prestamo' });
			}
		});
	} catch (error) {
		console.log(error);
...
exports.updatePrestamos = (req, res) => {
	try {
		const { id } = req.params;
		const { fecha, devolucion, entrega, modified } = req.body;
		let query = `UPDATE prestamos SET fecha = '${fecha}', devolucion ='${devolucion}', entrega='${entrega}', modified = '${modified}'
							WHERE id = ${id}`;
